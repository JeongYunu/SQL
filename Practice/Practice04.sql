...
select loc.location_id,
       ...
       loc.state_province,
       loc.country_id
from locations loc, (select de.location_id
                     from employees em, departments de
                     where em.department_id = de.department_id
                     and first_name = 'Steven'
                     and last_name = 'King') fName
where loc.location_id = fName.location_id;
...
select em.employee_id,
       ...
       em.salary,
       em.department_id
from employees em, (select max(salary) maxSalary,
                           department_id
                    from employees
                    group by department_id) oem
where em.department_id = oem.department_id
and em.salary = oem.maxSalary
order by salary desc;
...
select jo.job_title,
       ss.sumsalary
from jobs jo, (select sum(salary) sumSalary,
                       job_id
                from employees
                group by job_id) ss
where jo.job_id = ss.job_id;
